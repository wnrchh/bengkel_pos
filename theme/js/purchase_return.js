      ...
      selectInitial :true,*/
        ...
        select: function (e, ui) { 
            ...
            if(parseFloat(stock)==0){
              toastr["warning"]("Only 0 Items in Stock!!");
              return false;
            }
...
function return_row_with_data(item_id){
  ...
  if(!document.getElementById("purchase_id")){
    ...
    var purchase_id=$("#purchase_id").val().trim()
  }
  /*end*/
...
function restrict_quantity(item_id) {
    ...
    var selected_item_id = 0;
      ...
      if(available_qty!=0 && count_item_qty>=available_qty){
        toastr["warning"]("Only "+available_qty+" Items in Stock to Return!!");
...
function delete_return(q_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $.post("purchase_return/delete_return",{q_id:q_id},function(result){
   //alert(result);return;
     if(result=="success")
        {
          toastr["success"]("Record Deleted Successfully!");
...
function pay_now(purchase_id){
  $.post('purchase_return/show_pay_now_modal', {purchase_id: purchase_id}, function(result) {
...
function view_payments(purchase_id){
  $.post('purchase_return/view_payments_modal', {purchase_id: purchase_id}, function(result) {
...
function save_payment(return_id){
  var base_url=$("#base_url").val().trim();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $.post('purchase_return/save_payment', {return_id: return_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
